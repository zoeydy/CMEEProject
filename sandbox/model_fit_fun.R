...
sample_inits <- function(data, samp.time, samp.fact){
  ...
  t.low <- data$Time[which.min(abs(data$logN-pop_low))]
  t.high <- data$Time[which.min(abs(data$logN-pop_high))]
  midle_df <- data[data$Time > t.low & data$Time < t.high,]
  ...
  if (nrow(midle_df)<2) {
    ...
    # plot(midle_df$Time, midle_df$logN)
    # lines(time.r, N.predict)
    
    time.r <- seq(min(midle_df$Time), max(midle_df$Time), length.out = 100)
    N.predict <- coef(lm_fit)[1] + coef(lm_fit)[2]*time.r
    tlagStart <- data$Time[which.min(abs(N.predict-data$logN))] 
