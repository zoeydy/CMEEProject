...
total2 <-  merge(df,qf,by="discrete_curve")

total2<- select(total2, -c('"PopBioLog"', '"Time"'))

files3 <- read.csv('../Results/Formated/Total_processedLog.csv')


#format data
qf2 <- files3 %>%
  nest(-Temp,-Species,-Medium,-Rep,-Citation,-Time_units,-PopBio_unts,data = c("Time","PopBioLog", "PopBio"))

qf2$discrete_curve <-  seq.int(nrow(qf2))

qf2 <- unnest(qf2, 'data')

total3 <-  merge(df,qf2,by="discrete_curve")



#save
write.csv(total3,"../Results/Formated/3Total_processed_Log_RVK_3_models.csv", row.names = FALSE)
